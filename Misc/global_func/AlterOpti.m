...
tau_begin=10; tau_stop=300;% Pavia
% tau_begin=15; tau_stop=35;% Pavia whole
step=(tau_stop-tau_begin)/10;
...
RMSE_sub_ini=mean(mean((VX_dec-VX_real).^2));
RMSE_org_ini=mean(mean((P_inc*(VX_dec-VX_real)).^2));
...
%% ADMM parameters
iters = 2e1;   %Iters for the external loop    
ADMM='on';admm_iters = 75; %tol_cost=1e-5;cost_fun=zeros(1,iters);temp3=zeros([nr nc nb_sub]);
...
RMSE_sub(t)=mean(mean((VX_dec-VX_real).^2));
RMSE_org(t)=mean(mean((P_inc*(VX_dec-VX_real)).^2));
